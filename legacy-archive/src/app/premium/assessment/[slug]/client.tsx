...
export default function DynamicAssessmentPage() {
  ...
  useEffect(() => {
    const fetchScore = async () => {
      console.log('🔍 Fetching score - isInvitedAssessment:', isInvitedAssessment, 'invitationData:', invitationData, 'invitationToken:', invitationToken);
      
      // If we have an invitation token, wait for invitation data to be loaded
      if (invitationToken && !invitationData) {
        console.log('⏳ Waiting for invitation data to load...');
        return;
      }
    ...
    fetchScore();
  }, [user?.u_id, skipCheck, isInvitedAssessment, invitationData, invitationToken]);
