...
export async function POST(req: Request) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.NEXT_PUBLIC_BASE_URL || 'https://optimaliq.ai'}/subscribe/thank-you?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${process.env.NEXT_PUBLIC_BASE_URL || 'https://optimaliq.ai'}/subscribe`,
      metadata: {
        u_id,
        plan,
        billingCycle,
