...
export default function OnboardingAssessmentPage() {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/tier2/assessments/leadership", {
        method: "POST",
      ...
      if (!response.ok || result.leadershipScore === undefined) {
        console.error("❌ Scoring API failed:", result);
        alert("Something went wrong while scoring the assessment.");
        return;
      }

      const leadershipScore = result.leadershipScore;

      // Step 2: Insert into Supabase
      const { error } = await supabase
        .from("leadership_assessment")
        .insert([{ ...sanitizedAnswers, score: leadershipScore, u_id: user.u_id }]);
  ...
  if (loading) return <div className="p-10 text-center">Loading your assessment...</div>;
