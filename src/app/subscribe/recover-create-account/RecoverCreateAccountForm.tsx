...
export default function RecoverCreateAccountForm() {
  const router = useRouter();
  const [showModal, setShowModal] = useState(false);

  // Pre-fill data from localStorage for recovery
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    toast.success("🎉 Account completed! Redirecting to login...");
    setTimeout(() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <LabeledInput label="LinkedIn URL (optional)" name="linkedin_url" value={formState.linkedin_url} onChange={handleChange} />
