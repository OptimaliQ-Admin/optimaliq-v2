...
const GrowthDescriptions: Record<number, string> = {
...
export default function GrowthCard({ score, lastTakenDate, userId }: Props) {
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-lg p-6 space-y-4 transition hover:shadow-xl">
        <h2 className="text-xl font-semibold text-gray-800">📊 Growth & Benchmarking Intake</h2>
        ...
        {hasTaken && (
          <>
            ...
            <p className="text-gray-600">{GrowthDescriptions[roundedScore ?? 1]}</p>
      ...
      {showIntro && (
        <AssessmentIntroModal
          ...
          onStart={() => {
            setShowIntro(false);
            router.push("/premium/assessment/competitive-benchmarking");
          }}
