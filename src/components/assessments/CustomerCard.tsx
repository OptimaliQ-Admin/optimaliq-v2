...
const CustomerDescriptions: Record<number, string> = {
...
export default function CustomerCard({ score, lastTakenDate, userId }: Props) {
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-lg p-6 space-y-4 transition hover:shadow-xl">
        <h2 className="text-xl font-semibold text-gray-800">👥 Customer Experience Assessment</h2>
        ...
        {hasTaken && (
          <>
            ...
            <p className="text-gray-600">{CustomerDescriptions[roundedScore ?? 1]}</p>
      ...
      {showIntro && (
        <AssessmentIntroModal
          ...
          onStart={() => {
            setShowIntro(false);
            router.push("/premium/assessment/customer-experience");
          }}
          assessmentType="cx"
        />
      )}
